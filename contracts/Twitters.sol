...
contract Twitters is ReentrancyGuard{
    ...
    mapping(address => uint) public addToId;
    ...
    function fetchUserTweets(address _userAd) public view returns (Tweet[] memory) {
        uint256 tweetsCount = _tweet.current();
        uint256 tweetsFetch = 0;
        uint256 currentIndex = 0;

        for(uint i =0; i<tweetsCount; i++) {
            if(idToTweet[i+1].publisher == _userAd){
                tweetsFetch++;
            }
        }
    ...
    function userLength() public view returns (uint){
        return _newUser.current();
    }

    function isFollowed(address _userAd) public view returns ( bool){
        for(uint i =0; i<following[msg.sender].length; i++){
            if(following[msg.sender][i] ==_userAd){
                return true;
            }
        }
        return false;
